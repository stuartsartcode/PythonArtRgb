...
class Rgb(tuple):
    def __new__(self, *rgba: tuple):
        ...
        if len(rgba) == 0:
            ...
            if isinstance(rgba[0], str):
                self.rgb = tuple(
                    max(0, min(255, int(x))) for x in ImageColor.getrgb(rgba[0])
                )
            else:
    ...
    def soft_light_ps(self, r):
        return Rgb(denorm(soft_light_ps(norm(self), norm(r))))

    def avg(self, r):
        return Rgb(denorm(avg(norm(self), norm(r))))
...
def div(l, r):
    return bound(a if b <= 0 else a / b for a, b in zip(l, r))
...
def soft_light(l, r):
    return bound((1 - 2 * b) * a * a + 2 * b * a for a, b in zip(l, r))
...
def inv_soft_light(l, r):
    return bound(
        1 if (-2 * a**2 + 2 * a) == 0 else (c - a**2) / (-2 * a**2 + 2 * a)
        for a, c in zip(l, r)
    )
